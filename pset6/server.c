...
int main(int argc, char* argv[])
{
    ...
    while (true)
    {
        ...
        if (connected())
        {
            ...
            // break request-line into 3 parts
            char *noConLine = line;
            char *linePointer;
            char *method, *requestTarget, *HTTPVersion;
            
            method = strtok_r(noConLine, " ", &linePointer);
            requestTarget = strtok_r(NULL, " ", &linePointer);
            HTTPVersion = strtok_r(NULL, " ", &linePointer);
                        
            // printf("%s, %s, %s", method, requestTarget, HTTPVersion);
            
            // error checking, there should be no more spaces.
            // char* extraSpace = strstr(line, " ") + 1;
            ...
            // printf("%s %s", method, requestTarget);
            
            // extract query from request-target
            char *apptr;
            char *absolutePath;
            char *query = "";
            // ensure that there is a query to be obtained first!
            if (strstr(requestTarget, "?") != NULL)
            {
                absolutePath = strtok_r(requestTarget, "?", &apptr);
                query = strtok_r(NULL, "?", &apptr);
            ...
            // if there's no . in the web address, well how many have you seen without a . ?
            if (strchr(requestTarget, '.') == NULL)
            {
                error(501);
                continue;
            ...
            // concatenates root and absolute-path
            //printf("%s", root);
            
            char path[strlen(root) + strlen(absolutePath) + 2];
            strcpy(path, root);
            strcat(path, absolutePath);
            
            //printf("%s", path);

            /* ensures that path exists - credit to 
            * http://stackoverflow.com/questions/9314586/c-faster-way-to-check-if-a-directory-exists
            */
            struct stat pathExists;
            if (stat(path, &pathExists) == -1)
            {
                error(404);
                continue;
            }
            
            /* ensures path is readable - credit to
            * http://stackoverflow.com/questions/20648793/checking-if-file-path-is-readable-and-writable
            */
            ...
            char* extension = strstr(absolutePath, ".") + 1;
...
const char* lookup(const char* extension)
{
    ...
    // Time to MIME without reguard to case!
